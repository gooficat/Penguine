...
ShaderResource::ShaderResource(const string& path, const string& type) :
...
void ShaderResource::load() {
    ...
    if (type == "vertex")
        ...
        std::cerr << "currently we only support vertex, geometry and fragment shaders! type was \'" << type << "\'" << std::endl;
        return;
    }
